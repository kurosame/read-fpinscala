...
object Chapter8 {
  ...
  /** EXERCISE 8.18
    *
    * takeWhileが満たさなければならない他のプロパティを考え出せ。
    * takeWhileとdropWhileの関係をうまく表すプロパティを思いつけるか。
    */

  /** EXERCISE 8.19
    *
    * どのIntを返すかを選択するために何らかの方法で引数を使用する関数を生成したい。
    * これをうまく表現する方法を思いつけるか。
    * これは自由解答方式の難易度の高い問題である。
    * この問題から何を発見できるか。
    * 本章で開発してきたライブラリに組み込むことができる一般的なソリューションはあるか。
    */

  /** EXERCISE 8.20
    *
    * 本章で開発してきたライブラリを思い切って試してみる。
    * それを使って他に何がテストできるか確認し、新しい使用法、さらに拡張する方法、あるいはより便利なものにする方法を見つけ出せ。
    * 出発点として以下が参考になるだろう。
    *
    * ・take、drop、filter、unfoldなど、ListとStreamのために記述した他の関数の振る舞いを指定するプロパティを記述する。
    * ・第3章で定義したTreeデータ型を生成するためのサイズに基づくジェネレータを記述し、それを使って、Treeのために定義したfold関数の振る舞いを指定する。
    *  これが容易になるようにAPIを改良する方法を思いつけるか。
    * ・OptionとEitherのために定義したsequence関数の振る舞いを指定するプロパティを記述する。
    */
}
