...
object Chapter9 {
  ...
  /** EXERCISE 9.16
    *
    * ParseErrorをユーザーが利用するのに適した書式にする方法を考え出せ。
    * 選択肢はいろいろあるが、エラーをStringとして表示する際、ユーザーが同じ場所に関連付けられているラベルを結合する、またはグループ化したいと考えることが鍵となる。
    */

  /** EXERCISE 9.17
    *
    * sliceコンビネータには効率化の余地がある。
    * たとえば、`many(char('a')).slice`は依然として削除されるだけのList[Char]を生成する。
    * Parserの表現を修正してsliceを効率化する方法を思いつけるか。
    */

  /** EXERCISE 9.18
    *
    * パーサーをorコンビネータで結合すると失われる情報がある。
    * パーサーが2つとも失敗した場合は、2つ目のパーサーのエラーだけが保持される。
    * しかし、エラーメッセージを両方とも表示したい場合や、2つの分岐のうち長くもちこたえたほうのエラーを選択したい場合があるかもしれない。
    * ParseErrorの表現を変更し、パーサーのもう一方の分岐で発生したエラーを追跡できるようにせよ。
    */
}
